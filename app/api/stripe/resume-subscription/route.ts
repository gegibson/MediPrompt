...
async function findCancelableSubscription(stripe: Stripe, user: {
  id: string;
  email?: string | null;
}) {
  try {
...
export async function POST() {
  ...
  const subscription = await findCancelableSubscription(stripe, user);
  ...
  if (!subscription.cancel_at_period_end) {
    ...
    return NextResponse.json({
      ...
      notice: "Subscription is already active with no cancellation scheduled.",
    });
  }
  ...
  try {
    ...
    console.error("Unable to resume Stripe subscription", error);

    if (error instanceof Stripe.errors.StripeError) {
      return NextResponse.json(
        {
          error: "Stripe rejected subscription update",
          hint: error.message,
        },
